...
int n,m,q,timer=0,component;
vector<int>artpoints,g[MAX];
...
vector<int> shortestpath(int s,int dest){
    queue<int> q;
    vector<bool> used(n);
    vector<int> d(n), p(n);
...
void solve(){
    ...
    while(q--){
        ...
        vector<int>path=shortestpath(a,b);
        for(int x:path){
            if(x==a||x==b)ans++;
            else if(artpoints[x])ans++;
        }
        cout<<ans<<endl;
    }
}
