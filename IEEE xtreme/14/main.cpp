...
{
	double val = (R.ff - P.ff)*(Q.ss-P.ss) - (R.ss-P.ss)*(Q.ff-P.ff);
    if(val==0.0)return 0;
    if(val<0.0)return 1; // left
    return 2; // right
}
void solve(){
    ...
    for(int i=0;i<A.size();i++){
        for(int j=i+1;j<A.size();j++){
            ...
            for(int k=0;k<B.size();k++){
                if(fun(A[i],A[j],B[k])==1)lft++;
                if(fun(A[i],A[j],B[k])==2)rgt++;
            }
            if(lft&&rgt)continue;
            int lft2 = 0 , rgt2 = 0;
            for(int k=0;k<A.size();k++){
                if(fun(A[i],A[j],A[k])==1)lft2++;
                if(fun(A[i],A[j],A[k])==2)rgt2++;
            }
            if(lft2&&rgt2)continue;
            if( (lft2 && !lft) ||(!lft2 && lft) || (rgt2 && !rgt)|| (!rgt2 && rgt)){
                ok=1;
                break;
            }
        }
