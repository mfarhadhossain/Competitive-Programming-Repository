...
vector<pair<double,pii>> matrix[10];
//ll solveZul(int idx,int cnt){
//    if(cnt==0)return 0;
//    if(idx>=matrix[1].size()){
//        if(cnt==0)return 0;
//        return 1e18;
//    }
//    ll res = solveZul(idx+1,cnt-matrix[1][idx].first)+
//}
ll get(int set1,int set2,int set3){
    if(set1>sz[1]||set1>sz[2]||set1>sz[3])return 1e18;
    if(set2>sz[4]||set2>sz[5]||set2>sz[6])return 1e18;
    if(set3>sz[6]||set3>sz[7]||set3>sz[9])return 1e18;
    ll res=0;
    {
        for(int i=1;i<=3;i++){
            int need = set1;
            ...
            for(int j=0;j<matrix[i].size();j++){
                ret+=matrix[i][j].ss.ss;
                need-=matrix[i][j].ss.ff;
                if(need<=0)break;
            }
            res+=ret;
        }
...
void solve(){
    ...
    for(int i=0;i<m;i++){
            ...
            int k,p;cin>>k>>p;
            int id = coins[s];
            double avg = (p*1.0)/(k*1.0);
            matrix[id].pb({avg,{k,p}});
            sz[id]+=k;
    }
    for(int i=1;i<=9;i++)sort(matrix[i].begin(),matrix[i].end());
    ll res = 1e18;
    for(int set1=0;set1<=n;set1++){
        for(int set2=0;set2<=n-set1;set2++){

            int set3 = n - (set1+set2);

            res=min(res,get(set1,set2,set3));
