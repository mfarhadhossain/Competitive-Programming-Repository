...
int n,m,c;
vector<string>v;
int go(int idx,int pu,int pd){
    if(idx==m)return 0;
    int res=go(idx+1,0,0);
    if(v[0][idx]=='.' && v[1][idx]=='.'){
        if(pu==0 or pd==0)
        res=res+go(idx+1,1,0);
        if(pu==0 or pd==0)
        res=res+go(idx+1,0,1);
        if(pu==0&&pd==0)
        res=res+go(idx+1,1,1);
    }
    else if(v[0][idx]=='.'&&v[1][idx]!='.'){
        if(pu==0 or pd==0)
            res=res+go(idx+1,1,0);
    }
    else if(v[1][idx]=='.'&&v[0][idx]!='.'){
