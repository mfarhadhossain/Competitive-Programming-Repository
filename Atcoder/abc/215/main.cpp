...
const ll MAX = 1e5 + 10, MOD = 998244353;
string s;
int n;
ll dp[1005][1 << 11][15];
ll go(int idx, int mask, int pr) {
	if (idx == n) {
		if (__builtin_popcount(mask) == 0)return 0;
		return 1LL;
	}
	if (dp[idx][mask][pr] != -1)
		return dp[idx][mask][pr];
	ll ret = go(idx + 1, mask, pr) % MOD;
	int curr = s[idx] - 'A';
	if (mask & (1 << curr)) { // age nisi
		if (pr == curr) {
			ret = ret % MOD + go(idx + 1, mask, pr) % MOD;
			ret %= MOD;
		}
	}
	else {
		ret = ret % MOD + go(idx + 1, mask | (1 << curr), curr) % MOD;
		ret %= MOD;
	}
	//ret=ret+go(idx+1,mask,pr);
	return dp[idx][mask][pr] = ret % MOD;
}
void solve() {
	int n; cin >> n;
	ll res = -1e18;
