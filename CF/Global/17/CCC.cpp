...
#define    LB(a,x)         (lwer_bound(a.begin(),a.end(),x) - a.begin())
...
int rich[MAX], poor[MAX], n;

bool evaluate(int x) {
    //cout<<"for "<<tot<<": ";
    int rich_idx = x - 1, poor_idx = 0;

    int possible = 0;
    for (int i = 0; i < n; i++) {
        if (rich[i] >= rich_idx && poor[i] >= poor_idx) {
            possible++;
            rich_idx--;
            poor_idx++;
        }
...
void solve() {
    ...
    int ret = 1;
    for (int i = max(1, l - 10); i <= min(n, h + 10); i++) {
        if (evaluate(i)) {
            ret = max(ret, i);
        }
    }
