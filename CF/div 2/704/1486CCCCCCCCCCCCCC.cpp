...
int main ()
{
    ...
    while(r-l>1){
            int mid = (l+r)/2;
            int posL = query(l,mid-1);
            int posR = query(mid,r);

//            cout<<"l = "<<l<<" r = "<<r<<endl;
//            cout<<"second max = "<<second_max<<endl;
//
//            cout<<"posL = "<<posL<<" posR = "<<posR<<"\n";

            if(posL==second_max){ // max element is in [l...mid]
                r=mid-1;
                second_max=posL;
            }
            else{// max element is in [mid+1...r]
                l=mid;
                second_max=posR;
            }
    }
    ...
    if(r-l==1){
        ...
        if(l==second_max)cout<<"! "<<r<<"\n";
        else cout<<"! "<<l<<"\n";
    }
    else if(r-l==2){ // 1,3,2
        second_max = query(l,r);
        int posL = query(l,l+1);
        if(second_max==posL){
                r--;
                second_max = query(l,r);
                if(l==second_max)cout<<"! "<<r<<"\n";
                ...
                if(posL!=l && second_max!=l)cout<<"! "<<l<<"\n";
                if(posL!=r && second_max!=r)cout<<"! "<<r<<"\n";
                if(posL!=l+1 && second_max!=l+1)cout<<"! "<<l+1<<"\n";
        }
    }
    return 0;
}
