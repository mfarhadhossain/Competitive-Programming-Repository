...
int A[MAX],n,dp[MAX][2];
int recurr(int idx,int turn){
    if(idx>n)return 0;
    ...
    if(turn==0){ // bondhu
            ...
            cost = cost1 + cost2;
            ret = min(ret,cost + recurr(idx+2,turn^1));
    }
    else{

            ret = recurr(idx+1,turn^1);
            ret = min(ret,recurr(idx+2,turn^1));
    }
    return dp[idx][turn] = ret;
}
void solve(){
    cin>>n;
    for(int i=1;i<=n;i++){
