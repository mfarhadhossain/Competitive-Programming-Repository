...
void solve(){
    a = 0 , b = 0;
    cin>>n>>x;
    vector<ll>v(n),barabo,komabo;
    for(int i=0;i<n;i++)cin>>v[i];
    for(int i=0;i<n;i++){
        //cout<<x-v[i]<<" baraite hobe\n";
         if(v[i]>x){a+=v[i]-x;komabo.pb(v[i]-x);}
         if(v[i]<x){b+=x-v[i];barabo.pb(x-v[i]);}
    }
    if(a==b&&a==0)cout<<"0\n";
    else if(a==b)cout<<"1\n";
    else {
...
//    sort(komabo.rbegin(),komabo.rend());
//    sort(barabo.rbegin(),barabo.rend());
//    while(komabo.size()&&barabo.size()){
//        int x = komabo.back();
//        int y = barabo.back();
//        cout<<x<<" "<<y<<endl;
//        komabo.pop_back();
